...
This installation will create a volume called: `broadcaster-lpData` and it will
be located at `/var/lib/docker/volumes/broadcaster-lpData/_data`

### Container Folders

Within the docker container, the volume `broadcaster-lpData` will be mounted at
`/root/.lpData`
...
# Create Livepeer Broadcaster ETH account

In this step we need to start the Broadcaster in order to create an Ethereum
account.
...
livepeer_cli -host broadcaster -http 5935
```

Jump to
[Configure Transcoding Options](/broadcasters/guides/transcoding-options) to
finish configuring the Broadcaster
